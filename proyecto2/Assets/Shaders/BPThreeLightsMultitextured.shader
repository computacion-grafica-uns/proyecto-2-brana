Shader "BPThreeLightsMultitextured" {
    Properties{
        ...
        _MainTex_A("Texture", 2D) = "white" {}
        _MainTex_B("Texture", 2D) = "white" {}
        _TextureBlend("Textures Blend Factor", float) = 0.5
        ...
        SubShader{
            ...
            Pass {
                ...
                sampler2D _MainTex_A, _MainTex_B;
                float3 _CameraPos;
                ...
                fixed4 frag(v2f i) : SV_Target{
                    ...
                    float2 newUV_A = TRANSFORM_TEX(i.uv, _MainTex_A);
                    float2 newUV_B = TRANSFORM_TEX(i.uv, _MainTex_B);
                    float4 LowerTextureColor = tex2D(_MainTex_A, newUV_A);
                    float4 UpperTextureColor = tex2D(_MainTex_B, newUV_B);
                    float4 DiffuseColor = lerp((1+_TextureBlend) * LowerTextureColor, UpperTextureColor, _TextureBlend);
                    // float4 DiffuseColor = LowerTextureColor + UpperTextureColor;
